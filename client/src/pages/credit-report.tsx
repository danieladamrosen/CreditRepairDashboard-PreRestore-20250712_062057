...
const CreditAccountsCollapsedCard = ({ negativeAccounts, positiveAccounts, savedDisputes, creditAccountsCollapsed, onExpand }: {
  ...
  return (
    <Card className={`cursor-pointer transition-all duration-300 hover:shadow-lg rounded-lg ${
      ...
      (!hasUnsavedNegativeAccounts && Object.keys(savedDisputes).some(key => savedDisputes[key]) ? 'border-2 border-green-600 bg-green-50' : 
      'border border-gray-200 bg-white hover:border-gray-300')
    } overflow-hidden`}>
      <CardHeader
        className={`cursor-pointer ${hasUnsavedNegativeAccounts ? 'bg-red-50 hover:bg-red-100' : (!hasUnsavedNegativeAccounts && Object.keys(savedDisputes).some(key => savedDisputes[key]) ? 'bg-green-50 hover:bg-green-100' : 'hover:bg-gray-50')} transition-colors duration-200 rounded-lg`}
        onClick={onExpand}
      >
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-3">
            ...
            ) : (!hasUnsavedNegativeAccounts && Object.keys(savedDisputes).some(key => savedDisputes[key]) ? (
              <SavedCheckIcon />
            ) : (
            ...
            <div>
              <h3 className={`text-lg font-bold ${hasUnsavedNegativeAccounts ? 'text-red-700' : (!hasUnsavedNegativeAccounts && Object.keys(savedDisputes).some(key => savedDisputes[key]) ? 'text-green-700' : 'text-gray-700')}`}>
                {(!hasUnsavedNegativeAccounts && Object.keys(savedDisputes).some(key => savedDisputes[key])) ? 'Credit Accounts – All Negative Account Disputes Saved' : 'Credit Accounts'}
              </h3>
              <p className={`text-sm ${hasUnsavedNegativeAccounts ? 'text-red-700' : (!hasUnsavedNegativeAccounts && Object.keys(savedDisputes).some(key => savedDisputes[key]) ? 'text-green-700' : 'text-gray-600')} mt-1`}>
                {hasUnsavedNegativeAccounts ? getUnsavedItemsMessage() : (!hasUnsavedNegativeAccounts && Object.keys(savedDisputes).some(key => savedDisputes[key]) ? `You've saved disputes for all ${negativeAccounts.length} negative account${negativeAccounts.length > 1 ? 's' : ''} across TransUnion, Equifax, and Experian` : `${negativeAccounts.length} negative account${negativeAccounts.length !== 1 ? 's' : ''} may be impacting your credit score`)}
              </p>
            </div>
          </div>
...
export default function CreditReportPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/data/testing-credit-report.json');
  ...
  useEffect(() => {
    ...
    if (allNegativeAccountsSaved && !negativeAccountsCollapsed && !userHasManuallyExpanded) {
      ...
      scrollToSection('negative-accounts', () => {
        setNegativeAccountsCollapsed(true);
      });
    }
  }, [savedDisputes, creditData, negativeAccountsCollapsed, scrollToSection]);

  // Credit Accounts auto-collapse effect - Collapse entire section after all negatives saved
  useEffect(() => {
    // Use the existing negativeAccounts from useMemo hook
                ...
                <div className="flex justify-between items-center w-full">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold ${
                      ...
                      {creditSummaryReviewSaved ? <CheckIcon className="w-4 h-4" /> : 'CS'}
                    </div>
                    <div>
