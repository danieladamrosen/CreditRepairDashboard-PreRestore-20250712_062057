...
const DisputeWrapper: React.FC<{ children: React.ReactNode; isDisputeSaved?: boolean }> = ({ children, isDisputeSaved = false }) => (
  <div className={`border-2 rounded-lg p-6 overflow-visible ${
    isDisputeSaved 
...
const NumCircle: React.FC<{ n: number }> = ({ n }) => (
...
export function AccountRow({
  ...
  // Card expansion state (controlled vs uncontrolled)
  const [internalExpanded, setInternalExpanded] = useState(false);
  const [userHasManuallyToggled, setUserHasManuallyToggled] = useState(false);

  // Update expanded logic to respect showAllDetails prop
  useEffect(() => {
    if (showAllDetails && isExpanded === undefined && !userHasManuallyToggled) {
      // Global "Show All Details" is active and this card is not externally controlled
      // and user hasn't manually toggled this specific card
      setInternalExpanded(true);
    } else if (!showAllDetails && isExpanded === undefined) {
      // Global "Show All Details" is disabled, reset to collapsed and clear manual toggle flag
      setInternalExpanded(false);
      setUserHasManuallyToggled(false);
    }
  }, [showAllDetails, isExpanded, userHasManuallyToggled]);

  const expanded = isExpanded ?? (showAllDetails && !userHasManuallyToggled ? true : internalExpanded);
  const toggleExpanded = () => {
    if (isExpanded !== undefined) {
      // controlled, do nothing - parent controls the state
    } else {
  ...
  const applyBestPracticeCombination = async (
    ...
    // Use unified typeText function for reason
    await typeText(combination.reason, setCustomReason, setIsTypingReason, 25);
    
    // Brief pause between reason and instruction
    await new Promise((resolve) => setTimeout(resolve, 400));
    
    // Use unified typeText function for instruction
    await typeText(combination.instruction, setCustomInstruction, setIsTypingInstruction, 15);
  ...
  const checkFormCompletionAndShowArrow = (
    ...
    const hasInstruction = customInstruction.trim() || overrideInstruction || selectedInstruction;
...
Due to these reporting inaccuracies and regulatory violations, I am requesting the permanent deletion of this tradeline from my credit report.`;
  ...
  const addAllViolations = async (_event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const complianceReason = `Use All ${aiViolations.length}`;
    const complianceInstruction = `Use All ${aiViolations.length}`;
  ...
  // Use flat account properties directly
  const getAccountBalance = () => {
    return parseInt(account['BalanceAmount'] || account['@_UnpaidBalanceAmount'] || account['@_CurrentBalance'] || '0');
  };

  const getAccountStatus = () => {
    return account['AccountStatusCode'] || account._CURRENT_RATING?.['@_Code'] || '1';
  };

  const getAccountOpenDate = () => {
    return account['AccountOpenedDate'] || account['@_AccountOpenedDate'] || '2020-01-01';
  };

  const getAccountReportedDate = () => {
    return account['AccountReportedDate'] || account['@_AccountReportedDate'] || '2024-01-01';
  };

  const getCreditorName = () => {
    return account._CREDITOR?.['@_Name'] || account['@_SubscriberName'] || 'Unknown Creditor';
  };

  // Check if account is reported to each bureau
  const isReportedToTransUnion = () => {
    return account.reportingBureaus?.TransUnion === true;
  };

  const isReportedToEquifax = () => {
    return account.reportingBureaus?.Equifax === true;
  };

  const isReportedToExperian = () => {
    return account.reportingBureaus?.Experian === true;
  };
  ...
  return (
    <Card
      className={`transition-all duration-75 rounded-lg ${
        hasAnyNegative
          ? 'border-0 bg-white'
          : hasSelections
    ...
    >
      <CardContent className={`px-4 md:px-6 ${hasAnyNegative ? ' pb-2' : 'pt-1 pb-2'}`}>
        ...
        {/* Numbered guidance for negative accounts only - now moved to DisputeWrapper */}

        {/* Account Header */}
        <div className="mb-4">
          {/* Mobile: No standalone button here anymore */}

          {/* Desktop: Show all three bureau headers - only for positive accounts */}
          {!hasAnyNegative && (
            ...
            <Dialog open={showMobileModal} onOpenChange={setShowMobileModal}>
              ...
              <DialogContent className="max-w-[95vw] max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-6">
                  {/* TransUnion Details */}
                  {account.reportingBureaus.TransUnion && (
                    ...
                    <div className="bg-white border border-cyan-200 rounded-lg p-4 mb-4 -mx-1">
                      ...
                      <h3 className="font-bold text-gray-900 text-center text-xl leading-tight">
                        {account._CREDITOR?.["@_Name"] || "No Creditor Name"}
                      </h3>
                    </div>
                      ...
                      {isPublicRecord() ? (
                        ...
                        <>
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            ...
                            <span className="text-gray-900 text-sm">
                              {account["@_AccountIdentifier"] || "N/A"}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Date Opened:</span>
                            <span className="text-gray-900 text-sm">
                              {formatDate(account.AccountOpenedDate || account["@_AccountOpenedDate"])}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Account Status:</span>
                            <span className="text-gray-900 text-sm">
                              {account.AccountStatusCode || account["@_AccountStatusType"] || "N/A"}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            ...
                            <span className="text-gray-900 text-sm">
                              {formatCurrency(account.BalanceAmount || account["@_CurrentBalance"] || "0")}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            ...
                            <span className="text-gray-900 text-sm">
                              {formatDate(account["@_LastActivityDate"])}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <div className="bg-white border border-red-200 rounded-lg p-4 mb-4 -mx-1">
                      ...
                      <h3 className="font-bold text-gray-900 text-center text-xl leading-tight">
                        {account._CREDITOR?.["@_Name"] || account["@_SubscriberName"] || "Unknown Creditor"}
                      </h3>
                    </div>
                    <div className="grid grid-cols-1 gap-2 text-xs">
                      {isPublicRecord() ? (
                        ...
                        <>
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Account #:</span>
                            <span className="text-gray-900 text-sm">{account["@_AccountIdentifier"] || "N/A"}</span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Date Opened:</span>
                            <span className="text-gray-900 text-sm">
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            ...
                            <span
                              className={`text-sm ${getPaymentStatusStyle(getStatusDescription(getAccountStatus()))}`}
                            >
                              {getStatusDescription(getAccountStatus())}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Credit Limit:</span>
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Balance:</span>
                            <span className="text-gray-900 text-sm">{formatCurrency(account.BalanceAmount || account["@_CurrentBalance"] || "0")}</span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
        ...
        {/* AI Violations Alert moved to below gray divider */}

        {/* AI Dispute Suggestions moved to below gray divider */}
      </CardContent>

      {/* Dispute Section Wrapper for negative accounts */}
      {hasAnyNegative && (
        <DisputeWrapper isDisputeSaved={isDisputeSaved}>
          {/* ---------- STEP 1 ---------- */}
          <section className="space-y-4">
            ...
            {/* Original Bureau Comparison Grid - moved down from above */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                >
                  <div className="flex-between mb-3">
                    <h4 className={`font-semibold ${isDisputeSaved ? 'text-green-700' : 'text-black'}`}>{getCreditorName()}</h4>
                    <Select
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">${getAccountBalance()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Status:</span>
                      <span
                        className={`font-medium ${getPaymentStatusStyle(getStatusDescription(getAccountStatus()))}`}
                      >
                        {getStatusDescription(getAccountStatus())}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{getAccountReportedDate()}</span>
                    </div>
                    ...
                    {expanded && (
                      <>
                        ...
                        <div className="flex justify-between">
                          <span className="text-gray-600">Late 30 Days:</span>
                          <span className="font-medium">{getAccountField('@_Late30DaysCount', '0')}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Late 60 Days:</span>
                          <span className="font-medium">{getAccountField('@_Late60DaysCount', '0')}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Late 90+ Days:</span>
                          <span className="font-medium">{getAccountField('@_Late90DaysCount', '0')}</span>
                        </div>
                        <div className="flex justify-between">
              ...
              {/* Equifax */}
              <div>
                <div className={`font-bold mb-1 ${isDisputeSaved ? 'text-green-700' : 'text-red-700'}`}>Equifax</div>
                <div
                  className={`border-3 rounded-lg p-4 ${
                    isDisputeSaved
                      ? 'border-green-500 bg-green-50'
              ...
              {/* Experian */}
              <div>
                <div className={`font-bold mb-1 ${isDisputeSaved ? 'text-green-700' : 'text-blue-800'}`}>Experian</div>
                <div
                  className={`border-3 rounded-lg p-4 ${
                    isDisputeSaved
                      ? 'border-green-500 bg-green-50'
            ...
            <div className="hidden md:flex justify-center -mt-2 mb-2 relative w-full">
              <button
          ...
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className={`font-bold ${isDisputeSaved ? 'text-green-700' : 'text-black'}`}>
                Create Dispute
                {isDisputeSaved && <Check className="inline-block w-4 h-4 ml-2 text-green-600" />}
              </span>
            </div>

            {/* Reason Selection */}
            <div className={`${isDisputeSaved ? 'bg-green-50 border border-green-500 rounded-lg p-4' : ''}`}>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="__custom__">✏️ Write custom reason...</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className={`${isDisputeSaved ? 'bg-green-50 border border-green-500 rounded-lg p-4' : ''}`}>
              ...
              >
                ...
                >
                  <SelectValue placeholder="Select dispute instruction..." />
                </SelectTrigger>
                <SelectContent>
                  ...
                  <SelectItem value="__custom__">✏️ Write custom instruction...</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </section>
          ...
          <section>
            <div className="flex gap-2 justify-between items-center pt-2">
                ...
                <div className="flex items-center gap-2 relative overflow-visible">
                  ...
                  {isDisputeSaved ? (
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-green-500 text-xs font-bold text-white mr-1">✓</span>
                  ) : (
                              ...
                              setTimeout(() => {
                                ...
                                for (const card of allAccountCards) {
                                  ...
                                  if (isCurrentAccount) {
                                    ...
                                    const isPinkCard =
                                      card.classList.contains('bg-red-50 border border-rose-200 rounded-t-xl') ||
                                      card.querySelector('.bg-red-50 border border-rose-200 rounded-t-xl');
